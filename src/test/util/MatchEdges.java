...
public class MatchEdges {
	...
	public MatchEdges(PAG pag) {
		...
		while (store_it.hasNext()) {
			...
			for (int i = 0; i < storeTargets.length; i ++) {
				...
				String storeTargetFieldSig = 
						((SootField)storeTargetField).getSignature(); // f's signature
				...
				if (storeTargetField instanceof SootField) {
					...
					while (load_it.hasNext()) {
						...
						if (loadSrcField instanceof SootField) {
							// see whether x.f and y.f has the same offset of the field
							String loadSrcFieldSig = 
									((SootField)loadSrcField).getSignature(); // f's signature
	...
	public Iterator<Object> matchSourcesIterator() { return match.keySet().iterator(); }
	public Iterator<Object> matchInvSourcesIterator() { return match.keySet().iterator(); }
	
	public Set<Object> matchSources() { return match.keySet(); }
	public Set<Object> matchInvSources() { return match.keySet(); }
