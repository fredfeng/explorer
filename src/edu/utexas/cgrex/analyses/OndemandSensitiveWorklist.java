...
public class OndemandSensitiveWorklist {
				...
				for (Node element0 : targets) {
					...
					target.getBase().makeP2Set().forall(new P2SetVisitor() {
						public final void visit(Node n) {
							...
							// my implementation
							// this is 100% correct
							Set<AllocNode> pt = me.sensitivePTAnalysis(src);
	...
	protected final boolean handleAllocNode(AllocNode src) {
		...
		for (Node element : targets) {
			...
			// my implementation
			// this is 100% correct
			Set<AllocNode> pt = me.sensitivePTAnalysis((VarNode) element);
			boolean succ = false;
			for (AllocNode alc : pt) {
	...
	protected final boolean handleVarNode(final VarNode src) {
			...
			while (addedEdges.hasNext()) {
				...
				if (addedSrc instanceof VarNode) {
					if (addedTgt instanceof VarNode) {
						...
						Set<AllocNode> pt = me
								.sensitivePTAnalysis((VarNode) edgeTgt);
						boolean succ = false;
						for (AllocNode alc : pt) {
		...
		for (Node element : storeTargets) {
			...
			ret = fr.getBase().getP2Set().forall(new P2SetVisitor() {
				public final void visit(Node n) {
					...
					Set<AllocNode> pt = me.sensitivePTAnalysis(nDotF);
					for (AllocNode alc : pt) {
						nDotF.makeP2Set().add(alc);
					}
				}
		...
		for (Node[] p : storesToPropagate) {
			...
			// my implementation
			// Set<AllocNode> pt = me.sensitivePTAnalysis(storeSource);
			// boolean succ = false;
			// for (AllocNode alc : pt) {
